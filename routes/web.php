...
use KKPhim\ThemeHhtq\Controllers\ThemeHhtqController;
...
Route::group([
    ...
    Route::post(sprintf('/%s/{movie}/{episode}-{id}/report', config('kkphim.routes.movie', 'phim')), [ThemeHhtqController::class, 'reportEpisode'])
        ->where(['movie' => '.+', 'episode' => '.+', 'id' => '[0-9]+'])->name('episodes.report');
    Route::post(sprintf('/%s/{movie}/rate', config('kkphim.routes.movie', 'phim')), [ThemeHhtqController::class, 'rateMovie'])->name('movie.rating');
